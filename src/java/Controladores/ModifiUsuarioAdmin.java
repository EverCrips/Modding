 ...
 * @author Ignacio
 */
public class ModifiUsuarioAdmin extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            String run = request.getParameter("runUser");
          ...
          if (co.modificarDatosAdmin(id, nombre, apellidos,run,celular,correo,pass)){
              HttpSession sesion = request.getSession(true);
              co=new ConsultasUsuario();
              sesion.setAttribute("ingresado", "True");
              request.getRequestDispatcher("index.jsp").forward(request, response);
          }
          else{
              HttpSession sesion = request.getSession(true);
              sesion.setAttribute("ERROR", "Error 102 | No se pudo modificar los datos de la carretera.");
